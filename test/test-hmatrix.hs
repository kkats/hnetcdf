...
hmatrixDataOrdering = do
  let xdim = NcDim "x" 5 False
      ...
      z1 = cmap realToFrac $ matrix 5 [1..50] :: Matrix Double
      z2 = fromColumns $ toColumns z1 :: Matrix Double
  let write nc = do
        let xvar = fromJust $ ncVar nc "x"
            yvar = fromJust $ ncVar nc "y"
            z1var = fromJust $ ncVar nc "z1"
            z2var = fromJust $ ncVar nc "z2"
        put nc xvar $ SV.fromList [1..5 :: Double]
        put nc yvar $ SV.fromList [1..10 :: Double]
  ...
  let read nc = do
        let z1var = fromJust $ ncVar nc "z1"
            z2var = fromJust $ ncVar nc "z2"
        Right (HMatrix z1) <- get nc z1var :: HMatrixRet Double
        Right (HMatrix z2) <- get nc z2var :: HMatrixRet Double
        return (z1, z2)
  (z1tst, z2tst) <-
